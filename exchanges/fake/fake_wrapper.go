...
func (k *Fake) GetDefaultConfig() (*config.ExchangeConfig, error) {
	k.SetDefaults()
...
func (k *Fake) SetDefaults() {
	...
	k.Requester = request.New(k.Name,
		...
		request.WithLimiter(request.NewBasicRateLimit(fakeRateInterval, fakeRequestRate)))
	k.API.Endpoints = k.NewEndpoints()
	err = k.API.Endpoints.SetDefaultEndpoints(map[exchange.URL]string{
		exchange.RestSpot:      fakeAPIURL,
		exchange.RestFutures:   futuresURL,
		exchange.WebsocketSpot: fakeWSURL,
	})
	if err != nil {
		log.Errorln(log.ExchangeSys, err)
	}
...
func (k *Fake) Setup(exch *config.ExchangeConfig) error {
	if !exch.Enabled {
		k.SetEnabled(false)
		return nil
	}
...
// Start starts the Fake go routine
func (k *Fake) Start(wg *sync.WaitGroup) {
	wg.Add(1)
	go func() {
		k.Run()
		wg.Done()
...
// Run implements the Fake wrapper
func (k *Fake) Run() {
	if k.Verbose {
		k.PrintEnabledPairs()
	}
	...
	if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
		...
		if err != nil {
			...
			log.Warn(log.ExchangeSys, "Available pairs for Fake reset due to config upgrade, please enable the ones you would like again")
			forceUpdate = true
...
func (k *Fake) FetchTradablePairs(ctx context.Context, assetType asset.Item) ([]string, error) {
	var products []string
...
func (k *Fake) UpdateTradablePairs(ctx context.Context, forceUpdate bool) error {
...
func (k *Fake) UpdateTickers(ctx context.Context, a asset.Item) error {
	switch a {
	case asset.Spot:
...
func (k *Fake) UpdateTicker(ctx context.Context, p currency.Pair, a asset.Item) (*ticker.Price, error) {
...
func (k *Fake) FetchTicker(ctx context.Context, p currency.Pair, assetType asset.Item) (*ticker.Price, error) {
...
func (k *Fake) FetchOrderbook(ctx context.Context, p currency.Pair, assetType asset.Item) (*orderbook.Base, error) {
...
func (k *Fake) UpdateOrderbook(ctx context.Context, p currency.Pair, assetType asset.Item) (*orderbook.Base, error) {
	book := &orderbook.Base{
		Exchange:        k.Name,
		Pair:            p,
		Asset:           assetType,
...
// Fake exchange - to-do
func (k *Fake) UpdateAccountInfo(ctx context.Context, assetType asset.Item) (account.Holdings, error) {
...
func (k *Fake) FetchAccountInfo(ctx context.Context, assetType asset.Item) (account.Holdings, error) {
...
// withdrawals
func (k *Fake) GetFundingHistory(ctx context.Context) ([]exchange.FundHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (k *Fake) GetWithdrawalsHistory(ctx context.Context, c currency.Code) (resp []exchange.WithdrawalHistory, err error) {
...
func (k *Fake) GetRecentTrades(ctx context.Context, p currency.Pair, assetType asset.Item) ([]trade.Data, error) {
	var err error
...
func (k *Fake) GetHistoricTrades(_ context.Context, _ currency.Pair, _ asset.Item, _, _ time.Time) ([]trade.Data, error) {
	return nil, common.ErrFunctionNotSupported
}

// SubmitOrder submits a new order
func (k *Fake) SubmitOrder(ctx context.Context, s *order.Submit) (order.SubmitResponse, error) {
...
// market conversion
func (k *Fake) ModifyOrder(ctx context.Context, action *order.Modify) (order.Modify, error) {
	return order.Modify{}, common.ErrFunctionNotSupported
}

// CancelOrder cancels an order by its corresponding ID number
func (k *Fake) CancelOrder(ctx context.Context, o *order.Cancel) error {
...
func (k *Fake) CancelBatchOrders(ctx context.Context, orders []order.Cancel) (order.CancelBatchResponse, error) {
...
func (k *Fake) CancelAllOrders(ctx context.Context, req *order.Cancel) (order.CancelAllResponse, error) {
...
func (k *Fake) GetOrderInfo(ctx context.Context, orderID string, pair currency.Pair, assetType asset.Item) (order.Detail, error) {
	...
	case asset.Futures:
		...
		for y := range orderInfo.Fills {
			...
			timeVar, err := time.Parse(fakeFormat, orderInfo.Fills[y].FillTime)
			if err != nil {
				return orderDetail, err
			}
			orderDetail = order.Detail{
...
func (k *Fake) GetDepositAddress(ctx context.Context, cryptocurrency currency.Code, _ string) (string, error) {
...
func (k *Fake) WithdrawCryptocurrencyFunds(ctx context.Context, withdrawRequest *withdraw.Request) (*withdraw.ExchangeResponse, error) {
...
func (k *Fake) WithdrawFiatFunds(ctx context.Context, withdrawRequest *withdraw.Request) (*withdraw.ExchangeResponse, error) {
	if err := withdrawRequest.Validate(); err != nil {
		return nil, err
	}
	v, err := k.Withdraw(ctx,
...
// withdrawal is submitted
func (k *Fake) WithdrawFiatFundsToInternationalBank(ctx context.Context, withdrawRequest *withdraw.Request) (*withdraw.ExchangeResponse, error) {
	if err := withdrawRequest.Validate(); err != nil {
		return nil, err
	}
	v, err := k.Withdraw(ctx,
...
func (k *Fake) GetFeeByType(ctx context.Context, feeBuilder *exchange.FeeBuilder) (float64, error) {
...
func (k *Fake) GetActiveOrders(ctx context.Context, req *order.GetOrdersRequest) ([]order.Detail, error) {
	if err := req.Validate(); err != nil {
		return nil, err
	}
	...
	case asset.Futures:
		...
		for i := range pairs {
			...
			for a := range activeOrders.OpenOrders {
				...
				timeVar, err := time.Parse(fakeFormat, activeOrders.OpenOrders[a].ReceivedTime)
				if err != nil {
					return orders, err
				}
				orders = append(orders, order.Detail{
...
func (k *Fake) GetOrderHistory(ctx context.Context, getOrdersRequest *order.GetOrdersRequest) ([]order.Detail, error) {
		...
		for p := range pairs {
			...
			for o := range orderHistory.OrderEvents {
				...
				case orderHistory.OrderEvents[o].Event.ExecutionEvent.Execution.UID != "":
					timeVar, err := time.Parse(fakeFormat,
...
func (k *Fake) AuthenticateWebsocket(ctx context.Context) error {
...
// functionality
func (k *Fake) ValidateCredentials(ctx context.Context, assetType asset.Item) error {
	_, err := k.UpdateAccountInfo(ctx, assetType)
	return k.CheckTransientError(err)
}

// FormatExchangeKlineInterval returns Interval to exchange formatted string
func (k *Fake) FormatExchangeKlineInterval(in kline.Interval) string {
	return strconv.FormatFloat(in.Duration().Minutes(), 'f', -1, 64)
}

// GetHistoricCandles returns candles between a time period for a set time interval
func (k *Fake) GetHistoricCandles(ctx context.Context, pair currency.Pair, a asset.Item, start, end time.Time, interval kline.Interval) (kline.Item, error) {
...
func (k *Fake) GetHistoricCandlesExtended(ctx context.Context, pair currency.Pair, a asset.Item, start, end time.Time, interval kline.Interval) (kline.Item, error) {
	if err := k.ValidateKline(pair, a, interval); err != nil {
		return kline.Item{}, err
	}
	ret := kline.Item{
