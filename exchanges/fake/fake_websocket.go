...
const (
	fakeWSURL              = "wss://ws.fake.com"
	fakeAuthWSURL          = "wss://ws-auth.fake.com"
	fakeWSSandboxURL       = "wss://sandbox.fake.com"
	fakeWSSupportedVersion = "1.4.0"
	// WS endpoints
	fakeWsHeartbeat            = "heartbeat"
	fakeWsSystemStatus         = "systemStatus"
	fakeWsSubscribe            = "subscribe"
	fakeWsSubscriptionStatus   = "subscriptionStatus"
	fakeWsUnsubscribe          = "unsubscribe"
	fakeWsTicker               = "ticker"
	fakeWsOHLC                 = "ohlc"
	fakeWsTrade                = "trade"
	fakeWsSpread               = "spread"
	fakeWsOrderbook            = "book"
	fakeWsOwnTrades            = "ownTrades"
	fakeWsOpenOrders           = "openOrders"
	fakeWsAddOrder             = "addOrder"
	fakeWsCancelOrder          = "cancelOrder"
	fakeWsCancelAll            = "cancelAll"
	fakeWsAddOrderStatus       = "addOrderStatus"
	fakeWsCancelOrderStatus    = "cancelOrderStatus"
	fakeWsCancelAllOrderStatus = "cancelAllStatus"
	fakeWsRateLimit            = 50
	fakeWsPingDelay            = time.Second * 27
	fakeWsOrderbookDepth       = 1000
)
...
var defaultSubscribedChannels = []string{
	...
	fakeWsOHLC,
	fakeWsSpread}
var authenticatedChannels = []string{fakeWsOwnTrades, fakeWsOpenOrders}
...
func (k *Fake) wsFunnelConnectionData(ws stream.Connection, comms chan stream.Response) {
...
func (k *Fake) wsReadData(comms chan stream.Response) {
	defer k.Websocket.Wg.Done()
...
// success param = was the response from Fake successful or not
...
func (k *Fake) wsHandleData(respRaw []byte) error {
	if strings.HasPrefix(string(respRaw), "[") {
		...
		if event, ok := eventResponse["event"]; ok {
			switch event {
			case stream.Pong, fakeWsHeartbeat:
				return nil
			case fakeWsCancelOrderStatus:
			...
			case fakeWsCancelAllOrderStatus:
				var status WsCancelOrderResponse
				err := json.Unmarshal(respRaw, &status)
				if err != nil {
					return fmt.Errorf("%s - err %s unable to parse WsCancelOrderResponse: %s",
			...
			case fakeWsSystemStatus:
				...
				if systemStatus.Version > fakeWSSupportedVersion {
					log.Warnf(log.ExchangeSys,
						"%v New version of Websocket API released. Was %v Now %v",
						k.Name,
						fakeWSSupportedVersion,
						systemStatus.Version)
				}
			case fakeWsAddOrderStatus:
...
func (k *Fake) wsPingHandler() error {
...
func (k *Fake) wsAuthPingHandler() error {
	message, err := json.Marshal(pingRequest)
	if err != nil {
		return err
	}
...
func (k *Fake) wsReadDataResponse(response WebsocketDataResponse) error {
...
func (k *Fake) wsHandleAuthDataResponse(response WebsocketDataResponse) error {
	if chName, ok := response[1].(string); ok {
		switch chName {
		case fakeWsOwnTrades:
			return k.wsProcessOwnTrades(response[0])
		case fakeWsOpenOrders:
...
func (k *Fake) wsProcessOwnTrades(ownOrders interface{}) error {
...
func (k *Fake) wsProcessOpenOrders(ownOrders interface{}) error {
	if data, ok := ownOrders.([]interface{}); ok {
		for i := range data {
...
func (k *Fake) addNewSubscriptionChannelData(response *wsSubscription) {
...
func (k *Fake) wsProcessTickers(channelData *WebsocketChannelData, data map[string]interface{}) error {
...
func (k *Fake) wsProcessSpread(channelData *WebsocketChannelData, data []interface{}) {
...
func (k *Fake) wsProcessTrades(channelData *WebsocketChannelData, data []interface{}) error {
...
func (k *Fake) wsProcessOrderBook(channelData *WebsocketChannelData, data map[string]interface{}) error {
...
func (k *Fake) wsProcessOrderBookPartial(channelData *WebsocketChannelData, askData, bidData []interface{}) error {
	base := orderbook.Base{
		Pair:            channelData.Pair,
		Asset:           asset.Spot,
		VerifyOrderbook: k.CanVerifyOrderbook,
	}
	// Fake ob data is timestamped per price, GCT orderbook data is
...
func (k *Fake) wsProcessOrderBookUpdate(channelData *WebsocketChannelData, askData, bidData []interface{}, checksum string) error {
	update := buffer.Update{
		Asset:    asset.Spot,
		Pair:     channelData.Pair,
		MaxDepth: fakeWsOrderbookDepth,
	}
...
func (k *Fake) wsProcessCandles(channelData *WebsocketChannelData, data []interface{}) error {
...
func (k *Fake) GenerateDefaultSubscriptions() ([]stream.ChannelSubscription, error) {
...
func (k *Fake) Subscribe(channelsToSubscribe []stream.ChannelSubscription) error {
...
func (k *Fake) Unsubscribe(channelsToUnsubscribe []stream.ChannelSubscription) error {
...
func (k *Fake) wsAddOrder(request *WsAddOrderRequest) (string, error) {
	id := k.Websocket.AuthConn.GenerateMessageID(false)
	request.RequestID = id
	request.Event = fakeWsAddOrder
...
func (k *Fake) wsCancelOrders(orderIDs []string) error {
	id := k.Websocket.AuthConn.GenerateMessageID(false)
	request := WsCancelOrderRequest{
		Event:          fakeWsCancelOrder,
		Token:          authToken,
	...
	if cancelOrdersStatus[id].Error != "" || len(orderIDs) != successful { // strange Fake logic ...
		var reason string
...
func (k *Fake) wsCancelAllOrders() (*WsCancelOrderResponse, error) {
	id := k.Websocket.AuthConn.GenerateMessageID(false)
	request := WsCancelOrderRequest{
		Event:     fakeWsCancelAll,
		Token:     authToken,
		RequestID: id,
	}
