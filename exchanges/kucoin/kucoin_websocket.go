...
const (
	kucoinWSURL              = "wss://ws.kucoin.com"
	kucoinAuthWSURL          = "wss://ws-auth.kucoin.com"
	kucoinWSSandboxURL       = "wss://sandbox.kucoin.com"
	kucoinWSSupportedVersion = "1.4.0"
	// WS endpoints
	kucoinWsHeartbeat            = "heartbeat"
	kucoinWsSystemStatus         = "systemStatus"
	kucoinWsSubscribe            = "subscribe"
	kucoinWsSubscriptionStatus   = "subscriptionStatus"
	kucoinWsUnsubscribe          = "unsubscribe"
	kucoinWsTicker               = "ticker"
	kucoinWsOHLC                 = "ohlc"
	kucoinWsTrade                = "trade"
	kucoinWsSpread               = "spread"
	kucoinWsOrderbook            = "book"
	kucoinWsOwnTrades            = "ownTrades"
	kucoinWsOpenOrders           = "openOrders"
	kucoinWsAddOrder             = "addOrder"
	kucoinWsCancelOrder          = "cancelOrder"
	kucoinWsCancelAll            = "cancelAll"
	kucoinWsAddOrderStatus       = "addOrderStatus"
	kucoinWsCancelOrderStatus    = "cancelOrderStatus"
	kucoinWsCancelAllOrderStatus = "cancelAllStatus"
	kucoinWsRateLimit            = 50
	kucoinWsPingDelay            = time.Second * 27
	kucoinWsOrderbookDepth       = 1000
)
...
var defaultSubscribedChannels = []string{
	...
	kucoinWsOHLC,
	kucoinWsSpread}
var authenticatedChannels = []string{kucoinWsOwnTrades, kucoinWsOpenOrders}
...
func (k *Kucoin) wsFunnelConnectionData(ws stream.Connection, comms chan stream.Response) {
...
func (k *Kucoin) wsReadData(comms chan stream.Response) {
	defer k.Websocket.Wg.Done()
...
// success param = was the response from Kucoin successful or not
...
func (k *Kucoin) wsHandleData(respRaw []byte) error {
	if strings.HasPrefix(string(respRaw), "[") {
		...
		if event, ok := eventResponse["event"]; ok {
			switch event {
			case stream.Pong, kucoinWsHeartbeat:
				return nil
			case kucoinWsCancelOrderStatus:
			...
			case kucoinWsCancelAllOrderStatus:
				var status WsCancelOrderResponse
				err := json.Unmarshal(respRaw, &status)
				if err != nil {
					return fmt.Errorf("%s - err %s unable to parse WsCancelOrderResponse: %s",
			...
			case kucoinWsSystemStatus:
				...
				if systemStatus.Version > kucoinWSSupportedVersion {
					log.Warnf(log.ExchangeSys,
						"%v New version of Websocket API released. Was %v Now %v",
						k.Name,
						kucoinWSSupportedVersion,
						systemStatus.Version)
				}
			case kucoinWsAddOrderStatus:
			...
			case kucoinWsSubscriptionStatus:
...
func (k *Kucoin) wsPingHandler() error {
...
func (k *Kucoin) wsAuthPingHandler() error {
	message, err := json.Marshal(pingRequest)
	if err != nil {
		return err
	}
...
func (k *Kucoin) wsReadDataResponse(response WebsocketDataResponse) error {
...
func (k *Kucoin) wsHandleAuthDataResponse(response WebsocketDataResponse) error {
	if chName, ok := response[1].(string); ok {
		switch chName {
		case kucoinWsOwnTrades:
			return k.wsProcessOwnTrades(response[0])
		case kucoinWsOpenOrders:
...
func (k *Kucoin) wsProcessOwnTrades(ownOrders interface{}) error {
...
func (k *Kucoin) wsProcessOpenOrders(ownOrders interface{}) error {
	if data, ok := ownOrders.([]interface{}); ok {
		for i := range data {
...
func (k *Kucoin) addNewSubscriptionChannelData(response *wsSubscription) {
...
func (k *Kucoin) wsProcessTickers(channelData *WebsocketChannelData, data map[string]interface{}) error {
...
func (k *Kucoin) wsProcessSpread(channelData *WebsocketChannelData, data []interface{}) {
...
func (k *Kucoin) wsProcessTrades(channelData *WebsocketChannelData, data []interface{}) error {
...
func (k *Kucoin) wsProcessOrderBook(channelData *WebsocketChannelData, data map[string]interface{}) error {
...
func (k *Kucoin) wsProcessOrderBookPartial(channelData *WebsocketChannelData, askData, bidData []interface{}) error {
	base := orderbook.Base{
		Pair:            channelData.Pair,
		Asset:           asset.Spot,
		VerifyOrderbook: k.CanVerifyOrderbook,
	}
	// Kucoin ob data is timestamped per price, GCT orderbook data is
...
func (k *Kucoin) wsProcessOrderBookUpdate(channelData *WebsocketChannelData, askData, bidData []interface{}, checksum string) error {
	update := buffer.Update{
		Asset:    asset.Spot,
		Pair:     channelData.Pair,
		MaxDepth: kucoinWsOrderbookDepth,
	}
...
func (k *Kucoin) wsProcessCandles(channelData *WebsocketChannelData, data []interface{}) error {
...
func (k *Kucoin) GenerateDefaultSubscriptions() ([]stream.ChannelSubscription, error) {
...
func (k *Kucoin) Subscribe(channelsToSubscribe []stream.ChannelSubscription) error {
...
func (k *Kucoin) Unsubscribe(channelsToUnsubscribe []stream.ChannelSubscription) error {
...
func (k *Kucoin) wsAddOrder(request *WsAddOrderRequest) (string, error) {
	id := k.Websocket.AuthConn.GenerateMessageID(false)
	request.RequestID = id
	request.Event = kucoinWsAddOrder
...
func (k *Kucoin) wsCancelOrders(orderIDs []string) error {
	id := k.Websocket.AuthConn.GenerateMessageID(false)
	request := WsCancelOrderRequest{
		Event:          kucoinWsCancelOrder,
		Token:          authToken,
	...
	if cancelOrdersStatus[id].Error != "" || len(orderIDs) != successful { // strange Kucoin logic ...
		var reason string
...
func (k *Kucoin) wsCancelAllOrders() (*WsCancelOrderResponse, error) {
	id := k.Websocket.AuthConn.GenerateMessageID(false)
	request := WsCancelOrderRequest{
		Event:     kucoinWsCancelAll,
		Token:     authToken,
		RequestID: id,
	}
