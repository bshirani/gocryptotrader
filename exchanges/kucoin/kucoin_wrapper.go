...
func (k *Kucoin) GetDefaultConfig() (*config.ExchangeConfig, error) {
	k.SetDefaults()
...
func (k *Kucoin) SetDefaults() {
	...
	k.Requester = request.New(k.Name,
		...
		request.WithLimiter(request.NewBasicRateLimit(kucoinRateInterval, kucoinRequestRate)))
	...
	err = k.API.Endpoints.SetDefaultEndpoints(map[exchange.URL]string{
		...
		exchange.WebsocketSpot: kucoinWSURL,
	})
	if err != nil {
		log.Errorln(log.ExchangeSys, err)
	}
...
func (k *Kucoin) Setup(exch *config.ExchangeConfig) error {
	if !exch.Enabled {
		k.SetEnabled(false)
		return nil
	}
...
func (k *Kucoin) Start(wg *sync.WaitGroup) {
	wg.Add(1)
	go func() {
		k.Run()
		wg.Done()
...
func (k *Kucoin) Run() {
	...
	if !common.StringDataContains(enabled.Strings(), format.Delimiter) ||
		...
		if err != nil {
			...
			log.Warn(log.ExchangeSys, "Available pairs for Kucoin reset due to config upgrade, please enable the ones you would like again")
			forceUpdate = true
...
func (k *Kucoin) FetchTradablePairs(ctx context.Context, assetType asset.Item) ([]string, error) {
	var products []string
...
func (k *Kucoin) UpdateTradablePairs(ctx context.Context, forceUpdate bool) error {
...
func (k *Kucoin) UpdateTickers(ctx context.Context, a asset.Item) error {
	switch a {
	case asset.Spot:
...
func (k *Kucoin) UpdateTicker(ctx context.Context, p currency.Pair, a asset.Item) (*ticker.Price, error) {
...
func (k *Kucoin) FetchTicker(ctx context.Context, p currency.Pair, assetType asset.Item) (*ticker.Price, error) {
...
func (k *Kucoin) FetchOrderbook(ctx context.Context, p currency.Pair, assetType asset.Item) (*orderbook.Base, error) {
...
func (k *Kucoin) UpdateOrderbook(ctx context.Context, p currency.Pair, assetType asset.Item) (*orderbook.Base, error) {
	book := &orderbook.Base{
		Exchange:        k.Name,
		Pair:            p,
		Asset:           assetType,
...
// Kucoin exchange - to-do
func (k *Kucoin) UpdateAccountInfo(ctx context.Context, assetType asset.Item) (account.Holdings, error) {
...
func (k *Kucoin) FetchAccountInfo(ctx context.Context, assetType asset.Item) (account.Holdings, error) {
...
// withdrawals
func (k *Kucoin) GetFundingHistory(ctx context.Context) ([]exchange.FundHistory, error) {
	return nil, common.ErrFunctionNotSupported
}

// GetWithdrawalsHistory returns previous withdrawals data
func (k *Kucoin) GetWithdrawalsHistory(ctx context.Context, c currency.Code) (resp []exchange.WithdrawalHistory, err error) {
...
func (k *Kucoin) GetRecentTrades(ctx context.Context, p currency.Pair, assetType asset.Item) ([]trade.Data, error) {
	var err error
...
func (k *Kucoin) GetHistoricTrades(ctx context.Context, p currency.Pair, assetType asset.Item, start time.Time, end time.Time) ([]trade.Data, error) {
	var err error
	...
	for _, t := range tradeData.Trades {
		...
		// 	side = order.Sell
		// } else {
		// 	fmt.Println("kucoin get historical neither sell or buy")
		// }
		resp = append(resp, trade.Data{
			Exchange:     k.Name,
			CurrencyPair: p,
...
func (k *Kucoin) SubmitOrder(ctx context.Context, s *order.Submit) (order.SubmitResponse, error) {
...
// market conversion
func (k *Kucoin) ModifyOrder(ctx context.Context, action *order.Modify) (order.Modify, error) {
	return order.Modify{}, common.ErrFunctionNotSupported
}

// CancelOrder cancels an order by its corresponding ID number
func (k *Kucoin) CancelOrder(ctx context.Context, o *order.Cancel) error {
...
func (k *Kucoin) CancelBatchOrders(ctx context.Context, orders []order.Cancel) (order.CancelBatchResponse, error) {
...
func (k *Kucoin) CancelAllOrders(ctx context.Context, req *order.Cancel) (order.CancelAllResponse, error) {
...
func (k *Kucoin) GetOrderInfo(ctx context.Context, orderID string, pair currency.Pair, assetType asset.Item) (order.Detail, error) {
	...
	case asset.Futures:
		...
		for y := range orderInfo.Fills {
			...
			timeVar, err := time.Parse(kucoinFormat, orderInfo.Fills[y].FillTime)
			if err != nil {
				return orderDetail, err
			}
			orderDetail = order.Detail{
...
func (k *Kucoin) GetDepositAddress(ctx context.Context, cryptocurrency currency.Code, _ string) (string, error) {
...
func (k *Kucoin) WithdrawCryptocurrencyFunds(ctx context.Context, withdrawRequest *withdraw.Request) (*withdraw.ExchangeResponse, error) {
...
func (k *Kucoin) WithdrawFiatFunds(ctx context.Context, withdrawRequest *withdraw.Request) (*withdraw.ExchangeResponse, error) {
	if err := withdrawRequest.Validate(); err != nil {
		return nil, err
	}
	v, err := k.Withdraw(ctx,
...
// withdrawal is submitted
func (k *Kucoin) WithdrawFiatFundsToInternationalBank(ctx context.Context, withdrawRequest *withdraw.Request) (*withdraw.ExchangeResponse, error) {
	if err := withdrawRequest.Validate(); err != nil {
		return nil, err
	}
	v, err := k.Withdraw(ctx,
...
func (k *Kucoin) GetFeeByType(ctx context.Context, feeBuilder *exchange.FeeBuilder) (float64, error) {
...
func (k *Kucoin) GetActiveOrders(ctx context.Context, req *order.GetOrdersRequest) ([]order.Detail, error) {
	if err := req.Validate(); err != nil {
		return nil, err
	}
	...
	case asset.Futures:
		...
		for i := range pairs {
			...
			for a := range activeOrders.OpenOrders {
				...
				timeVar, err := time.Parse(kucoinFormat, activeOrders.OpenOrders[a].ReceivedTime)
				if err != nil {
					return orders, err
				}
				orders = append(orders, order.Detail{
...
func (k *Kucoin) GetOrderHistory(ctx context.Context, getOrdersRequest *order.GetOrdersRequest) ([]order.Detail, error) {
		...
		for p := range pairs {
			...
			for o := range orderHistory.OrderEvents {
				...
				case orderHistory.OrderEvents[o].Event.ExecutionEvent.Execution.UID != "":
					timeVar, err := time.Parse(kucoinFormat,
...
func (k *Kucoin) AuthenticateWebsocket(ctx context.Context) error {
...
// functionality
func (k *Kucoin) ValidateCredentials(ctx context.Context, assetType asset.Item) error {
	_, err := k.UpdateAccountInfo(ctx, assetType)
	return k.CheckTransientError(err)
}

// FormatExchangeKlineInterval returns Interval to exchange formatted string
func (k *Kucoin) FormatExchangeKlineInterval(in kline.Interval) string {
	return strconv.FormatFloat(in.Duration().Minutes(), 'f', -1, 64)
}

// GetHistoricCandles returns candles between a time period for a set time interval
func (k *Kucoin) GetHistoricCandles(ctx context.Context, pair currency.Pair, a asset.Item, start, end time.Time, interval kline.Interval) (kline.Item, error) {
	...
	// NOTE kucoin returns the next timestamp after the since, so we have to refer to the minute before
	...
	// fmt.Println("kucoin-gethistoriccandles received candles", len(candles), "for", pair, "firsttime:", decimal.NewFromFloat(candles[0].Time), decimal.NewFromFloat(candles[len(candles)-1].Time))
	for x := range candles {
	...
	// fmt.Println("kucoin-GETOHLC: minute requested", minRequested, "starting", start, "ending", end, ".received", len(ret.Candles))

	lastCandle := ret.Candles[len(ret.Candles)-1]
	minReceived := int(lastCandle.Time.Sub(ret.Candles[0].Time).Minutes())
	if (minRequested - 1) > minReceived {
		log.Errorln(log.SyncMgr, "ERROR",
			pair,
			"received",
			minReceived,
			"out of",
...
func (k *Kucoin) GetHistoricCandlesExtended(ctx context.Context, pair currency.Pair, a asset.Item, start, end time.Time, interval kline.Interval) (kline.Item, error) {
	...
	candles, err := k.GetOHLC(ctx, pair, k.FormatExchangeKlineInterval(interval), start.Unix())
	if err != nil {
		fmt.Println("!kucoin GETOHLC", err)
		return kline.Item{}, err
	}

	// kt, err := k.GetServerTime(ctx)
	// fmt.Println("kucoin server time is", kt, err)

	t1, err := convert.TimeFromUnixTimestampFloat(candles[0].Time * 1000)
	t2, err := convert.TimeFromUnixTimestampFloat(candles[len(candles)-1].Time * 1000)
	fmt.Println("candles received from kucoin", t1, t2, "requested", start, end)
	...
	for i := range candles {
		...
		if timeValue.Before(start) || timeValue.After(end) {
			// fmt.Printf(
			// 	"time is out of range candleTime:%f start:%d end:%d start:%s end:%s parsed:%s \n",
			// 	candles[i].Time,
			// 	start.Unix(),
			// 	end.Unix(),
			// 	start,
	...
	minRequested := int(end.Sub(start).Minutes())
	// fmt.Println("kucoin-OHLCEXTENDED: minute requested", minRequested, "starting", start, "ending", end, ". Only received", len(ret.Candles))

	if len(ret.Candles) == 0 {
		return ret, fmt.Errorf("no candles returned kucoin")
	}
	lastCandle := ret.Candles[len(ret.Candles)-1]
	minReceived := int(lastCandle.Time.Sub(ret.Candles[0].Time).Minutes())
	if minRequested > minReceived {
		fmt.Println("kucoinwrapperERROR", pair, "received", minReceived, "out of", minRequested, "startRequest", start, "startReceived", ret.Candles[0].Time, "endReceived", lastCandle.Time)
	}

	// fmt.Println(
	// 	"kucoin received candles",
	// 	pair,
	// 	len(ret.Candles),
	// 	ret.Candles[0].Time,
	// 	lastCandle)
