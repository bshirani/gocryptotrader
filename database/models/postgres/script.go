...
var ScriptTableColumns = struct {
	...
	LastExecutedAt: "script.last_executed_at",
...
type scriptR struct {
	ScriptExecutions ScriptExecutionSlice `boil:"ScriptExecutions" json:"ScriptExecutions" toml:"ScriptExecutions" yaml:"ScriptExecutions"`
}
...
func (scriptL) LoadScriptExecutions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeScript interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`script_execution`),
		qm.WhereIn(`script_execution.script_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
