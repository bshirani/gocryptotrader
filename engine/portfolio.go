...
func (p *Portfolio) ExecuteOrder(o order.Event, data data.Handler, om *OrderManager) (*fill.Fill, error) {
...
func (p *Portfolio) SetExchangeAssetCurrencySettings(exch string, a asset.Item, cp currency.Pair, c *ExchangeAssetPairSettings) {
	if c.ExchangeName == "" ||
		c.AssetType == "" ||
...
func (p *Portfolio) GetCurrencySettings(exch string, a asset.Item, cp currency.Pair) (ExchangeAssetPairSettings, error) {
	for i := range e.CurrencySettings {
...
func (p *Portfolio) GetAllCurrencySettings() ([]ExchangeAssetPairSettings, error) {
...
func (p *Portfolio) placeOrder(ctx context.Context, price, amount decimal.Decimal, useRealOrders, useExchangeLimits bool, f *fill.Fill, om *OrderManager) (string, error) {
	if f == nil {
		return "", eventtypes.ErrNilEvent
	}
	u, err := uuid.NewV4()
