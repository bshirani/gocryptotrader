...
func (p *PortfolioAnalysis) Save(fpath string) error {
	...
	fmt.Println("saving pf analysis", fpath)
	...
	fmt.Println("report", p.Report.Strategies[0].StartDate)
	payload, err := json.MarshalIndent(p.Report.Strategies, "", " ")

	if err != nil {
		fmt.Println("error marshalling json report", err)
		return err
	}
